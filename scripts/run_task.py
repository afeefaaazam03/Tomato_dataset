...
from tomato_multitask.engine import run_task


def main() -> None:
    parser = argparse.ArgumentParser(description="Run one tomato ripeness benchmark method.")
    parser.add_argument("--config", default="configs/tomato_benchmark.yaml")
    parser.add_argument("--task", choices=["classification", "segmentation", "counting", "detection"], required=True)
