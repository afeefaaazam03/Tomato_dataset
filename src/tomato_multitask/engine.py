...
from .plots import save_classification_samples, save_confusion_matrix, save_history_plot, save_prediction_overlay
...
def _optimizer(model: nn.Module, config: dict[str, Any]) -> torch.optim.Optimizer:
    ...
    return torch.optim.AdamW(params, lr=float(config.get("lr", 0.0003)), weight_decay=float(config.get("weight_decay", 0.05)))
...
# --------------------------------------------------------------------------------------
# Classification (primary task for the ripeness-grading use case)
# --------------------------------------------------------------------------------------
...
def run_classification(
    ...
    aug_cfg = config.get("augmentation", {}).get("classification", {})
    train_dataset = CropClassificationDataset(
        train_frame,
        image_size,
        augment=True,
        rotation_degrees=float(aug_cfg.get("rotation_degrees", 15.0)),
        jitter_strength=float(aug_cfg.get("jitter_strength", 0.15)),
        hue_jitter=float(aug_cfg.get("hue_jitter", 0.02)),
    )
    sampler = None
    shuffle_train = True
    ...
    for epoch in range(1, int(method_cfg.get("epochs", 40)) + 1):
        ...
        if _is_better(val_metric, score, best_value):
            ...
            best_epoch = epoch
            bad_epochs = 0
            torch.save({"model": model.state_dict(), "epoch": epoch, "classes": classes}, run_dir / "best.pt")
        else:
            bad_epochs += 1
    ...
    save_confusion_matrix(matrix, run_dir / "confusion_matrix_test.png", title="Tomato Ripeness -- Test Confusion Matrix")
    save_classification_samples(predictions, run_dir / "sample_predictions.png")
...
def run_counting(
    ...
    loader_kwargs = _loader_kwargs(config, device)
    train_loader = DataLoader(
        CountingDataset(_split(images, "train", limit), image_size, "total", augment=True),
        batch_size=int(method_cfg.get("batch_size", 12)),
        shuffle=True,
        **loader_kwargs,
    )
    val_loader = DataLoader(
        CountingDataset(_split(images, "val", limit), image_size, "total", augment=False),
        batch_size=int(method_cfg.get("batch_size", 12)),
        shuffle=False,
        **loader_kwargs,
    )
    test_loader = DataLoader(
        CountingDataset(_split(images, "test", limit), image_size, "total", augment=False),
        batch_size=int(method_cfg.get("batch_size", 12)),
        shuffle=False,
        **loader_kwargs,
    )
...
def _targets_to_numpy(targets: list[dict[str, torch.Tensor]]) -> list[dict[str, np.ndarray]]:
    return [{"boxes": t["boxes"].detach().cpu().numpy(), "labels": t["labels"].detach().cpu().numpy()} for t in targets]
