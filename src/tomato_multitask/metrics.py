...
def detection_metrics(predictions: list[dict], targets: list[dict], num_classes: int) -> tuple[dict[str, float], pd.DataFrame]:
    ...
    for class_id in range(1, num_classes + 1):
        ...
        per_class_rows.append(
            {"class_id": class_id, "ap50": float(ap50), "ap50_95": ap5095, "tp50": int(tp50), "fp50": int(fp50), "gt": int(gt50)}
        )
